import requests, json, base64, time, struct, re
...
# === HÀM protobuf_dec THAY THẾ protobuf_decoder ===
def protobuf_dec(data: str) -> str:
    """Giải mã protobuf từ hex string, trả về JSON"""
    try:
...
class APIClient:
    def __init__(self):
        ...
        self.language = "vn"
        self.base_url = detail_vers.get("server_url", "https://loginbp.ggpolarbear.com/")
        self.client_version = detail_vers.get("remote_version", "1.123.1")
        self.release_version = detail_vers.get("latest_release_version", "OB53")
    ...
    def GetLoginData(self):
        try:
            ...
            fields[3] = time.strftime("%Y-%m-%d %H:%M:%S")
            fields[7] = self.client_version
            fields[23] = int(tokendec.get("external_type", "")) if tokendec else 0
            fields[29] = str(tokendec.get("external_id", "")) if tokendec else ""
            fields[4] = "free fire"
            fields[5] = 4
            fields[8] = "Android OS 9 / API-28 (PI/rel.cjw.20220518.114133)"
            fields[9] = "Handheld"
            ...
            fields[42] = "WIFI"
            fields[57] = str(tokendec.get("signature_md5", "")) if tokendec else ""
            fields[60] = 32969
            fields[61] = 29665
            fields[62] = 2479
            fields[63] = 900
    ...
    def TAO_PACKET_XT(self) -> bytes:
        try:
            if not self.logindata:
                return b""
            esid = lambda rec: (
                lambda s: s[rec.upper()] if rec.upper() in s else None
            )({x["2"].upper(): x["1"] for x in self.logindata.get("19", [])})
            if not esid:
                return b""
            eid = hex(self._data.account_id)[2:] if self._data.account_id else "0"
            ...
            packet = "%s%s%s%X%05d%s%s" % (
                "%02d%02X" % (1, esid(self._data.server)),
                header,
                eid,
                self._data.login_time or 0,
                0x0,
                lengths,
                encrypts
            )
    ...
    def _build_api_response(self, authpacket):
        ...
        response["UserNickName"] = data.get("nickname") if data else ""
        response["UserAccountUID"] = data.get("account_id") if data else ""
        response["LockRegion"] = data.get("lock_region") if data else ""
        response["ClientVersion"] = data.get("client_version") if data else ""
        response["IsEmulator"] = data.get("is_emulator") if data else ""
